...
export const CommandSidebar: React.FC<CommandSidebarProps> = ({
  ...
  return (
    <aside className="w-full flex flex-col gap-4 bg-white border border-slate-200/80 rounded-2xl p-4 sm:p-5 shadow-xs">
      ...
      <div className="space-y-2 pb-2">
        <div className="relative">
          <select
            ...
            className="w-full bg-[#F4F6F9] border border-slate-200 rounded-xl px-3 py-2.5 text-xs font-semibold text-[#0B2545] focus:outline-none focus:ring-2 focus:ring-[#134074] appearance-none cursor-pointer transition-all"
          >
        ...
        <p className="text-[11px] text-slate-500 leading-normal px-0.5">
          {persona === "msme" && "Factory audits, sample testing, Udyam 50% marking concessions & STI."}
          {persona === "jeweler" && "6-digit HUID laser sync, AHC assay tariffs, and karatage rules."}
          {persona === "consumer" && "Guides on scanning HUID in BIS Care App and reporting substandard products."}
        </p>
      </div>
      ...
      <div className="space-y-1">
        ...
        <button
          ...
          className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold text-left transition-all ${
            activeTool === "audit-wizard"
              ...
              : "text-[#0B2545] hover:bg-slate-50 border border-transparent hover:border-slate-200/60"
          }`}
        >
        ...
        >
          ...
          <span className="text-[10px] text-slate-400 font-mono">100+ Standards</span>
        </button>

        <button
          onClick={() => {
            onSelectTool("fee-estimator");
            if (onCloseMobile) onCloseMobile();
          }}
          className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold text-left transition-all ${
        ...
        <div className="space-y-1.5">
          {persona === "msme" && (
            <>
              <button
                onClick={() => {
                  onTriggerPresetQuery("How to apply for ISI Mark under Scheme-I for MSME?");
                  if (onCloseMobile) onCloseMobile();
                }}
                className="w-full text-left text-[11px] text-slate-700 hover:text-[#134074] transition-colors block truncate"
              >
                • Scheme-I ISI Mark procedure
              </button>
              <button
                onClick={() => {
                  onTriggerPresetQuery("What is the 50% fee concession rule for Micro enterprises in BIS?");
                  if (onCloseMobile) onCloseMobile();
                }}
                className="w-full text-left text-[11px] text-slate-700 hover:text-[#134074] transition-colors block truncate"
              >
                • Udyam MSME 50% rebate
              </button>
              <button
                onClick={() => {
                  onTriggerPresetQuery("Check licensing requirements for IS 4151 Helmets");
                  if (onCloseMobile) onCloseMobile();
                }}
                className="w-full text-left text-[11px] text-slate-700 hover:text-[#134074] transition-colors block truncate"
              >
                • IS 4151 Helmets QCO mandate
              </button>
            </>
          )}
          ...
          {persona === "jeweler" && (
            <>
              <button
                onClick={() => {
                  onTriggerPresetQuery("How to get a Hallmarking license");
                  if (onCloseMobile) onCloseMobile();
                }}
                className="w-full text-left text-[11px] text-slate-700 hover:text-[#134074] transition-colors block truncate"
              >
                • Jeweller registration & ₹0 fee
              </button>
              <button
                onClick={() => {
                  onTriggerPresetQuery("What is 6-digit HUID and how does AHC laser engrave it?");
                  if (onCloseMobile) onCloseMobile();
                }}
                className="w-full text-left text-[11px] text-slate-700 hover:text-[#134074] transition-colors block truncate"
              >
                • 6-digit HUID protocol
              </button>
              <button
                onClick={() => {
                  onTriggerPresetQuery("What are the permitted gold purities under IS 1417:2016?");
                  if (onCloseMobile) onCloseMobile();
                }}
                className="w-full text-left text-[11px] text-slate-700 hover:text-[#134074] transition-colors block truncate"
              >
                • Karat fineness grades
              </button>
            </>
          )}
          ...
          {persona === "consumer" && (
            <>
              <button
                onClick={() => {
                  onTriggerPresetQuery("How to verify HUID and ISI mark using BIS Care App?");
                  if (onCloseMobile) onCloseMobile();
                }}
                className="w-full text-left text-[11px] text-slate-700 hover:text-[#134074] transition-colors block truncate"
              >
              ...
              <button
                onClick={() => {
                  onTriggerPresetQuery("How to report a fake ISI mark or counterfeit helmet?");
                  if (onCloseMobile) onCloseMobile();
                }}
                className="w-full text-left text-[11px] text-slate-700 hover:text-[#134074] transition-colors block truncate"
              >
                • Report fake ISI marks
              </button>
              <button
                onClick={() => {
                  onTriggerPresetQuery("Are all pressure cookers mandatory to have ISI mark?");
                  if (onCloseMobile) onCloseMobile();
                }}
                className="w-full text-left text-[11px] text-slate-700 hover:text-[#134074] transition-colors block truncate"
              >
                • Pressure cooker safety QCO
              </button>
            </>
          )}
        </div>
      ...
      <div className="space-y-1 pt-1 border-t border-slate-100">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex items-center justify-between px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:text-[#0B2545] hover:bg-slate-50 border border-slate-200/60 transition-colors"
        >
      ...
      {/* Footer Badge */}
      <div className="mt-auto pt-2 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-500">
